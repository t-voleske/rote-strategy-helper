...
*Getting your guild's numbers exactly right is tricky, especially regarding points gained through missions. In my experience, the results output by the helper usually are within one star of a well optimized RotE plan.*
...
1. A greedy short term approach, which always allocated points to the planet that needed the least points to the next star. 
...
3. Filtering all generated outcomes for different criteria led to a list of the most optimal outcomes to check each phase, which led to part one of the current strategy, only checking against a selected desirable outcomes each phase. This drastically cut down on branches created each phase. 
...
```
guild_save.txt              # persistent guild configuration (only present after saving a guild)
